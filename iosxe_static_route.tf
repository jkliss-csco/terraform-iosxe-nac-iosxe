    ...
    for device in local.devices : [
      for static_route in try(local.device_config[device.name].routing.static_routes, []) : {
        ...
        entries = [for e in try(static_route.entries, []) : {
          prefix      = try(e.prefix, local.defaults.iosxe.configuration.routing.static_routes.prefix, null)
          mask        = try(e.mask, local.defaults.iosxe.configuration.routing.static_routes.mask, null)
          next_hops   = try(e.next_hops, local.defaults.iosxe.configuration.routing.static_routes.next_hops, null)
        }
        ]
      }
    ]
